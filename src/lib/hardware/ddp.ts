...
// PUSH only on the last so the strip updates once, atomically.
export function ddpFramePackets(
  ...
  for (let off = 0; off < frame.length; off += MAX_DATA_BYTES) {
    ...
    packets.push(packet(chunk, off, last));
  }
  return packets;
}
...
export function ddpBlackoutPackets(pixelCount: number): Uint8Array[] {
