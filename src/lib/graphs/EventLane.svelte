<script lang="ts">
  ...
  // Events read outside the closure so changed content hands TimeAxis a new
  // draw function to repaint with.
  const drawTicks = $derived.by(() => {
    const list = events;
    const labeler = labelFor;
    return (u: uPlot) => {
      ...
      const labelBand = labeler ? 14 * dpr : 0;
