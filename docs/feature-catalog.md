...
- **Name** — canonical identifier used in the JSON profile. Snake_case. Per-stem features use a `{stem}_` prefix where `{stem}` is one of `kick`, `snare`, `hats`, `bass`, `synth`, `vocals`.
