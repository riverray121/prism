...
- **Library index:** `library.db` (SQLite) with one row per song — `id`, `title`, `artist`, `duration_sec`, `sample_rate`, `source_path`, `imported_at`, `queued_at`, `status`, `current_stage`, `current_stage_progress`, `analyzed_at`, `error_message`. SQLite is the standard for desktop apps of this shape (Mail, Photos, Lightroom, Firefox, Chrome all use it).
