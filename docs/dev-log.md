...
- **M1 polish — graph interaction.** Drag/click on the RMS graph scrubs the playhead (silenced during drag, resumes on release to avoid source-restart popping); wheel/trackpad zoom centered on cursor + Reset zoom button (drag-box zoom removed in favor of scrubbing); while playing+zoomed the view smoothly centers on the playhead. Graph enlarged (height 400, inspection view widened to max-w-4xl).
