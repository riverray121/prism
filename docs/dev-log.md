...
- **M1 slice 5 — Playback + playhead. M1 COMPLETE.** Asset protocol enabled (`protocol-asset` feature + `assetProtocol` scope `$HOME/**`); `profile.get` reply carries the absolute `audio_path` (runtime-only, kept out of profile.json). Playback uses an HTML `<audio>` element via `convertFileSrc`; a rAF loop reads `currentTime` to drive the playhead, drawn as a vertical line in `RmsGraph` via a uPlot draw hook. Play/pause + time display. Verified end-to-end: audio plays, playhead sweeps the RMS graph in sync.

## Todo

- **M1 polish (post-magic-moment)** — graph interaction: draggable playhead (seek), zoom in/out controls, auto-scroll graph to follow playhead when zoomed.
