...
- **Auto-map is a pure function behind an explicit action.** `lib/mapping/automap.ts`: favorited features → proposed starter doc (bindings chosen by feature kind: energy → brightness, onsets → gate, centroid → color_temp, heatmap → pixel row). Invoked only by the Generate button — never on open; a song starts with an empty doc and hand authoring is the primary flow. Filling an empty doc is silent; appending to a non-empty one requires confirmation, and it never removes or rewrites existing entries.
