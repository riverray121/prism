...
- **Onset derivation from intensity features.** Design a feature that takes an existing continuous intensity feature (RMS, onset strength, transient sharpness, band energy, …) and derives an `onsets` graph/table from it via peak-picking, with a user-adjustable intensity cutoff controlling which peaks are recorded as onsets. Interactive: pick the source feature and threshold, regenerate the onsets view live.
